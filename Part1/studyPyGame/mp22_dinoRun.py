...
pygame.display.set_caption('공룡달린다')
...
DUCKING = [pygame.image.load(f'{ASSETS}Dino/DinoDuck1.png'),
           pygame.image.load(f'{ASSETS}Dino/DinoDuck2.png')] # Duck=Dodge
JUMPING = pygame.image.load(f'{ASSETS}Dino/DinoJump.png')
START = pygame.image.load(f'{ASSETS}Dino/DinoStart.png') # 첫시작
DEAD = pygame.image.load(f'{ASSETS}Dino/DinoDead.png') # 죽음          
...
# 선인장이미지 로드 / 애니메이션을 위한게 아니라 종류가 세개씩인것!
...
class Dino: # 공룡 클래스
    ...
    def __init__(self) -> None:
        ...
        self.dino_run = True; self.dino_duck = False; self.dino_jump = False # 공룡 상태
    ...
    def update(self, userInput) -> None:
        ...
        elif userInput[pygame.K_DOWN] and not self.dino_jump: # 수루리
            self.dino_run = False
            self.dino_duck = True
    ...
    def run(self):
        self.image = self.run_img[self.step_index // 5] # 10 0, 1 왔다갔다 # run_img
...
class LargeCactus(Obstacle):
    def __init__(self, image) -> None:
        self.type = random.randint(0, 2) # 세개 중 하나를 고름
        super().__init__(image, self.type)
        self.rect.y = 300
...
def main():
    ...
    while run:
        ...
        cloud.update() # 구름이 공룡보다 먼저 그려져야 함(배경이니까)
        ...
        clock.tick(30) # 30-기본, 60-빨라짐
        pygame.display.update() # 초당 30초
